...
CREATE TABLE public.users (
  ...
  email text,
  firebase_uid text,
  civil_id text CHECK (civil_id ~ '^\d{12}$'::text),
  ...
  points_round_1 integer DEFAULT 0,
  points_round_2 integer DEFAULT 0,
  points_round_3 integer DEFAULT 0,
  points_round_4 integer DEFAULT 0,
  mcq_bonus_points integer DEFAULT 0,
...
CREATE TABLE public.mcq_questions (
  ...
  round_trigger USER-DEFINED NOT NULL,
...
CREATE TABLE public.mcq_answers (
  ...
  CONSTRAINT mcq_answers_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id),
  CONSTRAINT mcq_answers_question_id_fkey FOREIGN KEY (question_id) REFERENCES public.mcq_questions(id)
);
CREATE TABLE public.prize_winners (
  id uuid NOT NULL DEFAULT uuid_generate_v4(),
  prize_round USER-DEFINED NOT NULL UNIQUE,
  user_id uuid,
  prize_label text,
