'use client';import { useEffect, useState } from 'react';import Link from 'next/link';import { SCORING_TABLE, GROUPS } from '@/lib/constants';import api from '@/lib/api';import { useLanguage } from '@/context/LanguageContext';

function ArrowIcon() {return ();}

function TrophyIcon() {return ();}

const PRIZES = [{id: 'grand',label: 'Full Tournament',amount: '$1,000',desc: 'Top predictor across all rounds',isGrand: true,icon: '🏆',},{id: 'r1',label: 'Round 1',amount: 'Voucher',desc: 'Group Stage winner',icon: '⚽',},{id: 'r2',label: 'Round 2',amount: 'Voucher',desc: 'R32 & R16 best predictor',icon: '🎯',},{id: 'r3',label: 'Round 3',amount: 'Voucher',desc: 'Quarter & Semi-Finals winner',icon: '🔥',},{

export default function HomePage() {const [teams, setTeams] = useState([]);const { t, locale } = useLanguage();

useEffect(() => {api.get('/api/teams').then(res => {if (res.data) setTeams(res.data);}).catch(() => {});}, []);

const groupedTeams = GROUPS.map(g => ({letter: g,teams: teams.filter(tm => tm.group_letter === g),}));
